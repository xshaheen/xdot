...
#pragma warning restore IDE0130
	...
	public static class ClaimsPrincipalExtensions {
		/// <summary>Get the <paramref name="claimType"/> claim.</summary>
		public static Claim? Claim(this ClaimsPrincipal? principal, string claimType) {
			return principal?.FindFirst(claimType);
		}

		/// <summary>Get all claims of type <paramref name="claimType"/></summary>
		...
		/// <summary>Get user roles collection.</summary>
		...
		/// <summary>Get user roles as Enum collection of type <typeparamref name="T"/>.</summary>
		public static List<T>? Roles<T>(this ClaimsPrincipal? principal, string claimType = ClaimTypes.Role)
			where T : struct {
			return principal?.Claims
				.Where(c => c.Type == claimType)
				.Select(c => c.Value.ToEnum<T>())
				.ToList();
		}
	}
}
